...
const contractAddress = '0x5f03D1FAeFef678e10b739B727f3BAcE8158741F'; // Replace with your contract address
...
exports.uploadDocument = (req, res) => {
  upload.single('file')(req, res, function (err) {
    ...
    contractInstance.methods.uploadPatientReport(patientId, filePath)
      .send({ from: '0xE883C4e41EBDbe99d4944FDBc7694eC10B5D1a07', gas: 500000 }) // Replace with doctor's Ethereum address
...
exports.getPatientDetailsAndDocuments = (req, res) => {
  ...
  db.query(patientDetailsSql, [uuid], (err, patientDetailsResult) => {
    ...
    db.query(documentsSql, [uuid], (err, documentsResult) => {
      if (err) {
        return res.status(500).json({ error: 'Error fetching documents for patient' });
      }

      // Combine patient details and documents into a single response object
      const responseData = {
